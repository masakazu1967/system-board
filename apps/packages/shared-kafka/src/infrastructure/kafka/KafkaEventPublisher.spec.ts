...
import { DomainEvent } from '@system-board/shared';
...
describe('KafkaEventPublisher', () => {
  ...
  describe('onModuleInit', () => {
    it('should subscribe to all topics and connect to Kafka', async () => {
      ...
      kafkaClient.connect.mockResolvedValue({} as Producer);
  ...
  describe('publish', () => {
    ...
    it('should include all event metadata in payload', async () => {
      ...
      const payload = (emitCall[1] as Record<string, unknown>).value as Record<
        string,
        unknown
      >;
    ...
    it('should serialize occurredOn to ISO string', async () => {
      ...
      // Assert
      const payload = (
        kafkaClient.emit.mock.calls[0][1] as Record<string, unknown>
